...
+ (void)drawQRCode:(QRcode *)code context:(CGContextRef)ctx size:(CGFloat)size {
  ...
  for (int i = 0; i < width; ++i) {
    for (int j = 0; j < width; ++j) {
      if (*data & 1) {
        rectDraw.origin =
            CGPointMake((j + qrMargin) * zoom, (i + qrMargin) * zoom);
...
+ (UIImage *)qrImageForString:(NSString *)string imageSize:(CGFloat)size {
  ...
  QRcode *code =
      QRcode_encodeString([string UTF8String], 0, QR_ECLEVEL_L, QR_MODE_8, 1);
  if (!code) {
    return nil;
  }
  ...
  CGContextRef ctx = CGBitmapContextCreate(
      0, size, size, 8, size * 4, colorSpace, kCGImageAlphaPremultipliedLast);

  CGAffineTransform translateTransform =
      CGAffineTransformMakeTranslation(0, -size);
  CGAffineTransform scaleTransform = CGAffineTransformMakeScale(1, -1);
  CGContextConcatCTM(
      ctx, CGAffineTransformConcat(translateTransform, scaleTransform));
...
+ (UIImage *)twoDimensionCodeImage:(UIImage *)twoDimensionCode
  ...
  [[self avatarImage:avatarImage]
      drawInRect:CGRectMake((size.width - size2.width) / 2.0,
                            (size.height - size2.height) / 2.0, size2.width,
                            size2.height)];
...
+ (UIImage *)avatarImage:(UIImage *)avatarImage {
  UIImage *avatarBackgroudImage = [UIImage imageNamed:@"Icon"];
  CGSize size = avatarBackgroudImage.size;
  UIGraphicsBeginImageContext(size);

  [avatarBackgroudImage drawInRect:CGRectMake(0, 0, size.width, size.height)];
  [avatarImage
      drawInRect:CGRectMake(10, 10, size.width - 20, size.height - 20)];
